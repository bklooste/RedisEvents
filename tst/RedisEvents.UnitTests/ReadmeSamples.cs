...
/// <summary>
/// Every code sample printed in <c>library/src/RedisEvents/README.md</c>, compiled.
///
...
public static partial class ReadmeSamples
{
    ...
    #region typed-handler

    /// <summary>
    /// README — Typed handlers. <c>IBatchHandler&lt;T&gt;</c> receives an already-deserialised value
    /// instead of calling <c>Deserialize&lt;T&gt;</c> by hand.
    /// </summary>
    public sealed class TypedOrderHandler : IBatchHandler<Order>
    {
        public ValueTask HandleAsync(ReadOnlyMemory<StreamMsg<Order>> batch, CancellationToken ct)
        {
            for (var i = 0; i < batch.Length; i++)
            {
                ...
                StreamId id = batch.Span[i].Envelope.Id;
                _ = (order, id);
            }
    ...
    /// <summary>README — Typed handlers. Registration: the extra type-info argument is what turns on deserialisation.</summary>
    public static void RegisterTypedHandler(IHostApplicationBuilder builder)
    {
