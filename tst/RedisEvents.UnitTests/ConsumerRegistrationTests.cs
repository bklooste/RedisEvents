...
public class ConsumerRegistrationTests
{
    ...
    /// <summary>
    /// The typed overload carries a <see cref="StreamConsumerRegistration.WrapAsTypedHandler"/>
    /// closure alongside the ordinary <c>HandlerType</c> — everything else about the registration
    /// (config lookup, singleton registration) is identical to the untyped overload it mirrors.
    /// </summary>
    [Fact]
    [Trait("TestType", "UnitTest")]
    public void AddStream_typed_byTopic_carriesHandlerTypeAndTypedWrapper()
    {
        var builder = Builder(OneConsumer);
        builder.AddStream<TypedOrdersHandler, TypedOrder>("orders", TypedPublishJson.Default.TypedOrder);

        var registration = RegistryOf(builder).Consumers[0];

        registration.Options.Topic.Should().Be("orders");
        registration.HandlerType.Should().Be<TypedOrdersHandler>();
        registration.Handler.Should().BeNull();
    ...
    /// <summary>A typed handler class, for the typed registration assertions.</summary>
    public sealed class TypedOrdersHandler : IBatchHandler<TypedOrder>
    {
