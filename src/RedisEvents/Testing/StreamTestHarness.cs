...
public static class StreamTestHarness
{
    ...
    /// <summary>
    /// Sends a single message to a typed <see cref="IMessageHandler{T}"/> and awaits its completion —
    /// the typed sibling of the <see cref="IMessageHandler"/> overload above. Round-trips the message
    /// through <paramref name="typeInfo"/> exactly as <c>TypedMessageHandlerAdapter{T}</c> does, so
    /// what the handler sees here matches what it would see live.
    /// </summary>
    /// <typeparam name="T">The message type.</typeparam>
    /// <param name="handler">The handler under test.</param>
    /// <param name="message">The message to send.</param>
    /// <param name="typeInfo">The <see cref="JsonTypeInfo{T}"/> for serializing (and deserializing) the message.</param>
    /// <param name="options">Optional partition, key, correlation id and headers to stamp on the message.</param>
    /// <param name="ct">Cancellation token.</param>
    /// <exception cref="ArgumentNullException"><paramref name="handler"/> or <paramref name="typeInfo"/> is null.</exception>
    public static async ValueTask SendAsync<T>(
        ...
        where T : notnull
    {
        ...
        var msg = Build(message, typeInfo, options);
        var typed = new StreamMsg<T>(msg.Deserialize(typeInfo), msg);
        await handler.HandleAsync(in typed, ct).ConfigureAwait(false);
    }

    /// <summary>
    /// Sends a batch of messages to a typed <see cref="IBatchHandler{T}"/> as one call — the typed
    /// sibling of the <see cref="IBatchHandler"/> overload above.
    /// </summary>
    /// <typeparam name="T">The message type.</typeparam>
    /// <param name="handler">The handler under test.</param>
    /// <param name="messages">The messages in the batch, in order. An empty batch still calls the handler.</param>
    /// <param name="typeInfo">The <see cref="JsonTypeInfo{T}"/> for serializing (and deserializing) the messages.</param>
    /// <param name="options">Optional partition, key, correlation id and headers, applied to every message.</param>
    /// <param name="ct">Cancellation token.</param>
    /// <exception cref="ArgumentNullException"><paramref name="handler"/>, <paramref name="messages"/> or <paramref name="typeInfo"/> is null.</exception>
    public static async ValueTask SendBatchAsync<T>(
        ...
        where T : notnull
    {
        ...
        StreamMsg[] batch = new StreamMsg[messages.Count];
        for (var i = 0; i < messages.Count; i++)
        {
            batch[i] = Build(messages[i], typeInfo, options);
        }

        ReadOnlyMemory<StreamMsg> memory = batch;
        var values = memory.Deserialize(typeInfo);
        var typed = new StreamMsg<T>[batch.Length];
        for (var i = 0; i < batch.Length; i++)
        {
            typed[i] = new StreamMsg<T>(values[i], batch[i]);
        }
