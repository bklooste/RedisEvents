...
/// <summary>
/// Registration API for Orange.Lib.Streams. Every overload is additive and zero-config-safe: a
...
{
    private const string OptionsKey = "Orange.Lib.Streams:StreamOptions";
    private const string RegistryKey = "Orange.Lib.Streams:StreamRegistry";
    private const string PublisherDefaultsKey = "Orange.Lib.Streams:PublisherDefaults";
        ...
        if (producer.Buffered)
        {
            ...
            builder.Services.AddKeyedSingleton(topic, (IServiceProvider sp, object? _) =>
                new BufferedStreamPublisher(
                    ...
                    producer,
                    topicOptions,
                    sp.GetService<ILoggerFactory>()?.CreateLogger("Orange.Lib.Streams.Producer")));
        ...
        Func<ReadOnlyMemory<StreamMsg>, CancellationToken, ValueTask>? handler)
    {
        ...
        var registration = new StreamConsumerRegistration(consumer, name, handlerType, handler);
    ...
    private static StreamOptions Core(IHostApplicationBuilder builder)
    {
        ...
        builder.Services.AddSingleton(sp => new StreamsConnectionProvider(
            options,
            sp,
            sp.GetService<ILoggerFactory>()?.CreateLogger("Orange.Lib.Streams")));
        ...
        if (AnyBackgroundTrimInterval(options))
        {
            builder.Services.AddSingleton<IHostedService>(sp => new BackgroundTrimmer(
                ...
                sp.GetService<ILoggerFactory>()?.CreateLogger("Orange.Lib.Streams.Trimming")));
        }

        // IPositionStore is deliberately NOT registered (R-17 decision). The seam is worth keeping —
        // it is what NullPositionStore and the Persist=None path are written against — but
        // StreamConsumerHost constructs RedisPositionStore directly, so a registration here would be
        // resolvable, substitutable, and completely ignored: a service that replaced it would get its
        // positions written to Redis anyway. Registering it is one line, and it belongs in the same
        // change as the host resolving it (`services.GetService<IPositionStore>() ?? new
        // RedisPositionStore(redis)`); until then, an unwired registration is the misconfiguration,
        // not the fix.
        //
        // The health check is NOT registered here. R-25 moved StreamsHealthCheck (and the admin
        // endpoints) into Orange.Lib.Streams.Web so this library needs neither ASP.NET Core nor the
